<script setup>
import { onMounted, ref } from 'vue'

const today = ref(null)

onMounted(() => {
	today.value = new Date().getDate().toString().padStart(2, '0')
})
</script>
<template>
	<div class="calendar">
		<span class="calendar__date">{{ today }}</span>
		<svg
			xmlns="http://www.w3.org/2000/svg"
			version="1.1"
			xmlns:xlink="http://www.w3.org/1999/xlink"
			width="24"
			height="24"
			x="0"
			y="0"
			viewBox="0 0 323.358 323.358"
			style="enable-background: new 0 0 512 512"
			xml:space="preserve"
			class=""
		>
			<g>
				<path
					d="M48.863 323.358h225.632c23.653 0 42.875-19.222 42.875-42.875V66.827c0-23.653-19.222-42.875-42.875-42.875h-17.006V11.976c0-6.587-5.39-11.976-11.977-11.976s-11.976 5.389-11.976 11.976v11.976H89.822V11.976C89.822 5.389 84.432 0 77.845 0S65.869 5.389 65.869 11.976v11.976H48.863c-23.653 0-42.875 19.222-42.875 42.875v213.656c0 23.653 19.222 42.875 42.875 42.875zM29.941 66.827c0-10.419 8.503-18.922 18.922-18.922h17.006v11.976c0 6.587 5.389 11.976 11.976 11.976s11.976-5.389 11.976-11.976V47.905h143.715v11.976c0 6.587 5.389 11.976 11.976 11.976s11.976-5.389 11.976-11.976V47.905h17.006c10.419 0 18.922 8.503 18.922 18.922v34.971H29.941zm0 58.923h263.477v154.733c0 10.419-8.503 18.922-18.922 18.922H48.863c-10.419 0-18.922-8.503-18.922-18.922z"
					fill="currentColor"
					opacity="1"
					data-original="currentColor"
				></path>
			</g>
		</svg>
	</div>
</template>
<style>
.calendar {
	position: relative;
	display: inline-block;
}

.calendar__date {
	position: absolute;
	bottom: 1px;
	left: 50%;
	transform: translateX(-50%);
	color: inherit;
	font-weight: bold;
	font-size: 10px;
}
</style>
